...
// Normalize rows: 优先 DataSource；否则 props.data；兼容 { list } / { data } 嵌套
...
/** 本地分页 fallback（无 DataSource 时的静态数据）*/
...
// 最终展示的 rows：DataSource → 服务端已切片；否则若开启分页 → 客户端切片；否则原始
...
function handlePageChange(page: number) {
  ...
  // 联动：DataSource 存在时自动 setPage，触发 refresh；否则本地翻页
  if (props.dataSource) {
...
function handlePageSizeChange(pageSize: number) {
  ...
  // 联动：DataSource 存在时自动 setPageSize；否则本地
  if (props.dataSource) {
