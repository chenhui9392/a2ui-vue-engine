<!--
 * @Author: hui.chenn
 * @Description: Schema Debug Page - JSON 调试工具（支持 PageSchema + 旧扁平数组两种格式）
 *
 *   职责：
 *   - 编辑 JSON → 自动识别格式 → 渲染 A2UIRoot 预览
 *   - PageSchema 格式（含 page + components）→ createRuntime → A2UIRoot :runtime
 *   - 旧扁平数组格式（[{ id, component }]）→ A2UIRoot processMessage
 *   - 内置 CRUD Schema 预设，一键加载
 * @Date: 2026-07-06 10:00:00
-->
<template>
  <div class="playground-container">
    <!-- Header -->
    <div class="playground-header">
      <div class="header-left">
        <h1 class="title">A2UI Playground</h1>
        <el-tag type="info" size="small">JSON Debug Tool</el-tag>
        <el-tag v-if="runtimeMode" type="success" size="small">PageSchema 模式</el-tag>
        <el-tag v-else type="warning" size="small">扁平数组模式</el-tag>
      </div>
      <div class="header-right">
        <el-tooltip content="加载内置 CRUD PageSchema" placement="bottom">
          <el-button @click="loadCrudSchema">
            <el-icon><Files /></el-icon>
            CRUD Schema
          </el-button>
        </el-tooltip>
        <el-tooltip content="执行 JSON 渲染" placement="bottom">
          <el-button type="primary" @click="handleRun">
            <el-icon><VideoPlay /></el-icon>
            Run
          </el-button>
        </el-tooltip>
        <el-tooltip content="格式化 JSON 数据" placement="bottom">
          <el-button @click="handleFormat">
            <el-icon><Document /></el-icon>
            Format
          </el-button>
        </el-tooltip>
        <el-tooltip content="复制 JSON 到剪贴板" placement="bottom">
          <el-button @click="handleCopy">
            <el-icon><CopyDocument /></el-icon>
            Copy
          </el-button>
        </el-tooltip>
      </div>
    </div>

    <!-- Main Content -->
    <div class="playground-main">
      <!-- Left Panel: 分上下两部分 -->
      <div class="left-panel">
        <!-- 上半部分: JSON Editor (70%空间) -->
        <div class="editor-section">
          <div class="section-header">
            <span class="section-title">JSON Schema</span>
            <el-tag type="warning" size="small" v-if="jsonLines">Lines: {{ jsonLines }}</el-tag>
          </div>
          <div class="editor-body">
            <div class="code-editor">
              <div class="line-numbers">
                <span v-for="n in lineCount" :key="n" class="line-number">{{ n }}</span>
              </div>
              <div class="code-content" ref="codeContentRef">
                <pre class="code-highlight" ref="highlightRef"><code v-html="highlightedJson"></code></pre>
                <textarea
                  ref="textareaRef"
                  v-model="jsonContent"
                  class="code-input"
                  placeholder="粘贴 PageSchema 或扁平节点数组 JSON..."
                  spellcheck="false"
                  @scroll="handleScroll"
                  @keydown="handleKeydown"
                ></textarea>
              </div>
            </div>
          </div>
          <div class="error-panel" v-if="errorMessage">
            <el-alert :title="errorMessage" type="error" :closable="false" show-icon />
          </div>
        </div>

        <!-- 下半部分: Form Data (30%空间) -->
        <div class="data-section">
          <div class="section-header">
            <span class="section-title">{{ runtimeMode ? 'Runtime State' : 'Form Data' }}</span>
          </div>
          <div class="data-body">
            <div class="code-editor data-editor">
              <div class="line-numbers">
                <span v-for="n in dataLineCount" :key="n" class="line-number">{{ n }}</span>
              </div>
              <div class="code-content">
                <pre class="code-highlight"><code v-html="highlightedFormData"></code></pre>
              </div>
            </div>
          </div>
        </div>
      </div>

      <!-- Right: Preview -->
      <div class="preview-panel">
        <div class="panel-header">
          <span class="panel-title">Preview</span>
          <el-tag type="success" size="small" v-if="renderSuccess">Rendered</el-tag>
        </div>
        <div class="preview-body">
          <!-- Debug 栏：显示 runtime 状态 -->
          <div v-if="runtimeMode" class="debug-bar">
            runtime: {{ !!runtime }} | nodes: {{ runtime?.initialNodes?.length ?? 0 }} | key: {{ runtimeKey }}
          </div>
          <!-- PageSchema 模式：走 createRuntime -->
          <A2UIRoot
            v-if="runtimeMode && runtime"
            :key="runtimeKey"
            :runtime="runtime"
          />
          <!-- 扁平数组模式：走 processMessage -->
          <el-form v-else label-width="100px">
            <A2UIRoot
              ref="a2uiRootRef"
              :component-map="componentMap"
              @message="handleAction"
            />
          </el-form>
        </div>
      </div>
    </div>

    <!-- Status Bar -->
    <div class="playground-footer">
      <div class="footer-left">
        <span>Messages: {{ messageCount }}</span>
        <span v-if="lastRunTime">Last run: {{ lastRunTime }}</span>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, shallowRef, computed, onMounted, onUnmounted, nextTick } from 'vue'
import { ElMessage } from 'element-plus'
import { VideoPlay, Document, CopyDocument, Files } from '@element-plus/icons-vue'
import { A2UIRoot, defaultComponentMap, createRuntime } from 'a2ui-vue-engine'
import type { A2Node, ComponentMapper, FormDataResult, A2UIRuntime } from 'a2ui-vue-engine'
import { defaultJson } from '../../mock'
import { schema as crudSchema } from '../../schema/crud/page.schema'

// Refs
const a2uiRootRef = ref<InstanceType<typeof A2UIRoot>>()
const textareaRef = ref<HTMLTextAreaElement>()
const highlightRef = ref<HTMLElement>()
const codeContentRef = ref<HTMLElement>()
const jsonContent = ref('')
const errorMessage = ref('')
const renderSuccess = ref(false)
const messageCount = ref(0)
const lastRunTime = ref('')
const componentMap: ComponentMapper = { ...defaultComponentMap }

// PageSchema 模式状态
// 用 shallowRef 而非 ref：避免 Vue 对 runtime 对象做深度响应式，
// 否则 runtime.state（一个 Ref）会被自动解包成普通对象，导致 A2UIRoot 内 data.value 为 undefined
const runtime = shallowRef<A2UIRuntime>()
const runtimeMode = ref(false)
const runtimeKey = ref(0)  // 切换 runtime 时强制重新挂载 A2UIRoot

// 基础设施配置（PageSchema 模式使用，与 CRUD 页面一致）
const debugBaseURL = '/hinton-agent-mario-server/api'
const debugAuth = {
  cacheKey: 'hinton:__gauss_user',
  tokenPath: 'loginInfo.accessToken',
  headerName: 'authorization',
  scheme: 'Bearer',
}

// 行数计算
const lineCount = computed(() => {
  if (!jsonContent.value) return 1
  return jsonContent.value.split('\n').length
})
const jsonLines = computed(() => lineCount.value)

// Form Data / Runtime State 行数
const dataLineCount = computed(() => {
  if (!formDataJson.value) return 1
  return formDataJson.value.split('\n').length
})

// JSON 语法高亮
function highlightJson(jsonStr: string): string {
  if (!jsonStr) return ''
  let escaped = jsonStr.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
  const lines = escaped.split('\n')
  const highlightedLines = lines.map(line => {
    line = line.replace(/"([^"]+)"(\s*:)/g, '<span class="hl-key">"$1"</span>$2')
    line = line.replace(/:\s*"([^"]*)"/g, ': <span class="hl-string">"$1"</span>')
    line = line.replace(/:\s*(-?\d+\.?\d*)(\s*[,\]\}])/g, ': <span class="hl-number">$1</span>$2')
    line = line.replace(/:\s*(true|false)(\s*[,\]\}])/g, ': <span class="hl-boolean">$1</span>$2')
    line = line.replace(/:\s*(null)(\s*[,\]\}])/g, ': <span class="hl-null">$1</span>$2')
    return line
  })
  return highlightedLines.join('\n')
}

const highlightedJson = computed(() => highlightJson(jsonContent.value))

// 旧格式：从扁平节点提取 form data
function generateFormData(jsonData: any): FormDataResult {
  const form: Record<string, string> = {}
  if (Array.isArray(jsonData) && jsonData.length > 0 && jsonData[0].id && jsonData[0].component) {
    jsonData.forEach((node) => {
      if (node.value?.path) {
        const pathMatch = node.value.path.match(/\/form\/(.+)/)
        if (pathMatch) form[pathMatch[1]] = ''
      }
    })
  } else {
    function traverse(n: any) {
      if (!n) return
      const type = n.type || ''
      const props = n.props || {}
      if (type === 'a2-text-field' && props.prop) form[props.prop] = ''
      if (n.children && Array.isArray(n.children)) n.children.forEach(traverse)
    }
    traverse(jsonData)
  }
  return { form }
}

// Form Data / Runtime State JSON
const formDataJson = computed<string>(() => {
  try {
    // PageSchema 模式：展示 runtime.state
    if (runtimeMode.value && runtime.value) {
      return JSON.stringify(runtime.value.state.value, null, 2)
    }
    // 扁平数组模式：提取 form data
    const node = JSON.parse(jsonContent.value)
    const formData = generateFormData(node)
    return JSON.stringify(formData, null, 2)
  } catch {
    return ''
  }
})

const highlightedFormData = computed(() => highlightJson(formDataJson.value))

// 滚动同步
function handleScroll() {
  if (textareaRef.value && highlightRef.value && codeContentRef.value) {
    const scrollTop = textareaRef.value.scrollTop
    const scrollLeft = textareaRef.value.scrollLeft
    highlightRef.value.scrollTop = scrollTop
    highlightRef.value.scrollLeft = scrollLeft
    const lineNumbersEl = codeContentRef.value.parentElement?.querySelector('.line-numbers')
    if (lineNumbersEl) lineNumbersEl.scrollTop = scrollTop
  }
}

// 键盘处理（Tab 键插入 2 空格）
function handleKeydown(e: KeyboardEvent) {
  if (e.key === 'Tab') {
    e.preventDefault()
    const textarea = textareaRef.value
    if (!textarea) return
    const start = textarea.selectionStart
    const end = textarea.selectionEnd
    const value = textarea.value
    textarea.value = value.substring(0, start) + '  ' + value.substring(end)
    textarea.selectionStart = textarea.selectionEnd = start + 2
    jsonContent.value = textarea.value
  }
}

// Initialize with default content
onMounted(() => {
  jsonContent.value = defaultJson
})

// 检测是否为 PageSchema 格式
function isPageSchema(data: any): boolean {
  return data && typeof data === 'object' && !Array.isArray(data)
    && data.page && data.components
}

// Parse and validate JSON
function parseJsonContent(): any | null {
  const content = jsonContent.value.trim()
  if (!content) {
    errorMessage.value = 'JSON content is empty'
    return null
  }
  try {
    const node = JSON.parse(content)
    errorMessage.value = ''
    return node
  } catch (e) {
    errorMessage.value = `JSON parse error: ${e}`
    return null
  }
}

// Run rendering
function handleRun() {
  const node = parseJsonContent()
  if (!node) return

  try {
    if (isPageSchema(node)) {
      // PageSchema 模式：createRuntime → A2UIRoot :runtime
      runtimeMode.value = true
      runtime.value = createRuntime({
        schema: node,
        baseURL: debugBaseURL,
        auth: debugAuth,
      })
      runtimeKey.value++  // 强制 A2UIRoot 重新挂载
      console.log('[schema-debug] runtime created:', !!runtime.value)
      console.log('[schema-debug] initialNodes:', runtime.value?.initialNodes?.length, runtime.value?.initialNodes)
    } else {
      // 扁平数组模式：processMessage
      runtimeMode.value = false
      runtime.value = undefined
      nextTick(() => {
        if (a2uiRootRef.value) {
          a2uiRootRef.value.updateTree(null)
          a2uiRootRef.value.processMessage({ type: 'node', node: node as A2Node })
        }
      })
    }
    messageCount.value++
    lastRunTime.value = new Date().toLocaleTimeString()
    renderSuccess.value = true
    errorMessage.value = ''
    ElMessage.success('Rendered successfully!')
  } catch (e) {
    errorMessage.value = `Render error: ${e}`
    renderSuccess.value = false
    ElMessage.error(errorMessage.value)
  }
}

// 加载内置 CRUD PageSchema
function loadCrudSchema() {
  jsonContent.value = JSON.stringify(crudSchema, null, 2)
  errorMessage.value = ''
}

// Format JSON
function handleFormat() {
  try {
    const parsed = JSON.parse(jsonContent.value)
    jsonContent.value = JSON.stringify(parsed, null, 2)
    ElMessage.success('JSON formatted!')
  } catch (e) {
    ElMessage.error('Invalid JSON')
  }
}

// Copy JSON
async function handleCopy() {
  try {
    await navigator.clipboard.writeText(jsonContent.value)
    ElMessage.success('Copied to clipboard!')
  } catch (e) {
    ElMessage.error('Copy failed!')
  }
}

// Handle action events from A2UI (扁平数组模式)
function handleAction(payload: any) {
  console.log('A2UI Action:', payload)
  ElMessage.info(`Action: ${payload.type}`)
}

onUnmounted(() => {
  if (a2uiRootRef.value) a2uiRootRef.value.updateTree(null)
  runtime.value?.destroy()
})
</script>

<style scoped>
.playground-container {
  display: flex;
  flex-direction: column;
  height: 100vh;
  overflow: hidden;
  background: #f8f9fa;
}
.playground-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  height: 56px;
  padding: 12px 20px;
  background: #fff;
  border-bottom: 1px solid #e4e7ed;
  flex-shrink: 0;
}
.header-left { display: flex; align-items: center; gap: 12px; }
.title { font-size: 18px; font-weight: 600; color: #303133; margin: 0; }
.header-right { display: flex; align-items: center; gap: 8px; }
.playground-main { display: flex; flex: 1; min-height: 0; overflow: hidden; }
.left-panel {
  flex: 1;
  display: flex;
  flex-direction: column;
  min-width: 0;
  border-right: 1px solid #e4e7ed;
  overflow: hidden;
  background: #fff;
}
.editor-section {
  flex: 7;
  display: flex;
  flex-direction: column;
  min-height: 0;
  overflow: hidden;
  border-bottom: 1px solid #e4e7ed;
}
.data-section {
  flex: 3;
  display: flex;
  flex-direction: column;
  min-height: 0;
  overflow: hidden;
}
.section-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  height: 36px;
  padding: 8px 16px;
  background: #f0f2f5;
  border-bottom: 1px solid #e4e7ed;
  flex-shrink: 0;
}
.section-title { font-size: 13px; font-weight: 500; color: #606266; }
.editor-body, .data-body { flex: 1; min-height: 0; overflow: hidden; }
.code-editor {
  display: flex;
  height: 100%;
  background: #fff;
  font-family: 'Consolas', 'Monaco', 'Courier New', monospace;
  font-size: 14px;
  line-height: 1.5;
}
.line-numbers {
  width: 50px;
  padding: 8px 0;
  background: #fafafa;
  color: #909399;
  text-align: right;
  user-select: none;
  overflow-y: auto;
  overflow-x: hidden;
  flex-shrink: 0;
  border-right: 1px solid #e8e8e8;
  scrollbar-width: none;
  -ms-overflow-style: none;
}
.line-numbers::-webkit-scrollbar { display: none; }
.line-number {
  display: block;
  padding-right: 12px;
  height: 21px;
  line-height: 21px;
  font-size: 13px;
}
.code-content { flex: 1; position: relative; overflow: hidden; }
.code-highlight {
  position: absolute;
  top: 0; left: 0; right: 0; bottom: 0;
  padding: 8px 12px;
  margin: 0;
  color: #303133;
  white-space: pre;
  overflow: auto;
  font-size: 14px;
  line-height: 1.5;
  pointer-events: none;
}
.code-highlight code { display: block; white-space: pre; }
.code-input {
  position: absolute;
  top: 0; left: 0; right: 0; bottom: 0;
  padding: 8px 12px;
  background: transparent;
  color: transparent;
  caret-color: #303133;
  border: none;
  resize: none;
  outline: none;
  font-family: inherit;
  font-size: 14px;
  line-height: 1.5;
  white-space: pre;
  overflow: auto;
  tab-size: 2;
}
.code-input::selection { background: rgba(64, 158, 255, 0.25); }
:deep(.hl-key) { color: #0451a5; }
:deep(.hl-string) { color: #a31515; }
:deep(.hl-number) { color: #09885a; }
:deep(.hl-boolean) { color: #0451a5; }
:deep(.hl-null) { color: #0451a5; }
.data-editor { background: #fafbfc; }
.data-editor .line-numbers { background: #f5f6f7; }
.data-editor .code-content { overflow: auto; }
.data-editor .code-highlight {
  background: #fafbfc;
  pointer-events: auto;
  overflow: auto;
}
.error-panel { padding: 8px 16px; background: #fef0f0; flex-shrink: 0; }
.preview-panel {
  flex: 1;
  display: flex;
  flex-direction: column;
  min-width: 0;
  overflow: hidden;
  background: #fff;
}
.panel-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  height: 36px;
  padding: 8px 16px;
  background: #f0f2f5;
  border-bottom: 1px solid #e4e7ed;
  flex-shrink: 0;
}
.panel-title { font-size: 13px; font-weight: 500; color: #606266; }
.preview-body {
  flex: 1;
  min-height: 0;
  padding: 20px 24px;
  overflow: auto;
  background: #fff;
  display: flex;
  flex-direction: column;
  align-items: stretch;
  justify-content: flex-start;
}
.preview-body .el-form { width: auto; }
/* 确保 A2UIRoot 及其内部组件正确撑满宽度 */
.preview-body :deep(.a2ui-root) { width: 100%; min-height: 100px; }
.preview-body :deep(.a2-list),
.preview-body :deep(.a2-column),
.preview-body :deep(.a2-row) { width: 100%; min-width: 0; }
.debug-bar {
  font-size: 12px;
  color: #e6a23c;
  padding: 4px 8px;
  background: #fdf6ec;
  border: 1px solid #f5dab1;
  border-radius: 4px;
  margin-bottom: 8px;
  flex-shrink: 0;
}
.playground-footer {
  display: flex;
  justify-content: space-between;
  align-items: center;
  height: 28px;
  padding: 4px 20px;
  border-top: 1px solid #e4e7ed;
  flex-shrink: 0;
}
.footer-left { display: flex; gap: 20px; font-size: 12px; color: #909399; }
</style>
